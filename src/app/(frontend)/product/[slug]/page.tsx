...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const brandTitle = typeof product.brand === 'object' && product.brand !== null
    ...
    <div className="w-full min-h-screen bg-background text-foreground pb-28 md:pb-12">
      <nav className="border-b border-border bg-card py-3 sm:py-4">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex items-center gap-2 text-[10px] sm:text-xs font-semibold tracking-wider uppercase text-muted-foreground overflow-x-auto no-scrollbar">
          ...
          <Link href={`/category/${category.slug}`} className="hover:text-primary transition-colors flex-shrink-0">
            {category.title}
          </Link>
          <ChevronRight className="h-3 w-3 flex-shrink-0" />
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 sm:py-10 lg:px-8">
        <div className="grid grid-cols-1 gap-8 lg:gap-12 md:grid-cols-2">
          ...
          <div className="flex flex-col">
            <span className="text-xs sm:text-sm font-semibold tracking-widest text-primary uppercase">{brandTitle}</span>
            <h1 className="mt-2 text-2xl sm:text-3xl lg:text-4xl font-bold text-foreground leading-tight">{product.title}</h1>

            <div className="my-5 sm:my-6 border-b border-border" />

            <p className="text-muted-foreground text-sm sm:text-base leading-relaxed whitespace-pre-wrap">{product.description}</p>
                  ...
                  <table className="w-full text-xs sm:text-sm text-left">
                    <tbody>
                      {product.specifications.map((spec, idx) => (
                        <tr key={spec.id ?? idx} className={idx % 2 === 0 ? 'bg-background' : 'bg-card'}>
                          <td className="px-4 py-3 font-semibold text-muted-foreground w-1/3 border-b border-border uppercase tracking-wider">
                            {spec.key}
                          </td>
                          <td className="px-4 py-3 text-foreground border-b border-border">{spec.value}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div key={prod.id} className="flex flex-col p-3 sm:p-5 border border-border rounded-2xl bg-background hover:border-primary transition-colors group">
                  <Link href={`/product/${prod.slug}`} className="flex flex-col flex-grow gap-3">
                    ...
                    <div>
                      ...
                      <h3 className="text-sm sm:text-base font-bold text-foreground leading-tight mt-1 line-clamp-2">{prod.title}</h3>
                    </div>
                  </Link>
                  <div className="mt-3">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="relative h-11 w-11 flex-shrink-0 overflow-hidden rounded-lg bg-card border border-border">
              <Image src={getImageUrl(getProductMainImage(product))} alt={product.title} width={44} height={44} className="object-contain" />
            </div>
