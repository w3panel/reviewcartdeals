    ...
    <>
      <div className="px-4 mt-6 sm:mt-8 pb-32 md:pb-12">
        ...
        <div className="flex gap-4 sm:gap-6 overflow-x-auto no-scrollbar pb-6">
          ...
          >
            <div
              className={`flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 rounded-full transition-colors ${
          ...
          {categories.slice(0, 8).map((cat: Category) => (
            ...
            >
              <div
                className={`relative flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 rounded-full overflow-hidden transition-colors ${
        ...
        <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-6">
          ...
          {(selectedCategory || selectedBrands.length > 0) && (
            <button type="button" onClick={handleClearAll} className="flex-shrink-0 ml-1">
              <span className="text-[11px] font-bold text-primary border-b border-primary pb-0.5">
                Clear All
              </span>
            </button>
          )}
        ...
        {(selectedCategory || selectedBrands.length > 0) && (
          <div className="flex items-center gap-2 flex-wrap mb-6">
            ...
            {selectedBrands.map((brand) => (
              <span
                key={brand}
                className="inline-flex items-center px-3 py-1.5 rounded-full bg-muted text-xs font-medium text-foreground"
              >
                {brand}
                <button
                  type="button"
        ...
        <div className="flex items-center justify-between mb-6 border-t border-border pt-6">
          <span className="text-sm font-medium text-foreground">
            {isLoading ? (
              ...
              <>
                <span className="text-lg font-bold">{totalDocs.toLocaleString()}</span> products
              </>
            )}
          </span>
          <button
        ...
        {products.length === 0 && !isLoading ? (
          ...
          <div
            className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 transition-opacity ${isLoading ? 'opacity-50 pointer-events-none' : ''}`}
          >
            {products.map((prod) => (
              <div
                key={prod.id}
                className="flex flex-col bg-card rounded-2xl overflow-hidden border border-border group"
              >
                <Link href={`/product/${prod.slug}`} className="flex flex-col h-full">
                  <div className="relative w-full aspect-square bg-background flex items-center justify-center p-3 sm:p-4">
                    <button
                      type="button"
                      className="absolute top-2 right-2 z-10 w-8 h-8 rounded-full bg-card/80 backdrop-blur flex items-center justify-center"
                  ...
                  <div className="p-3 sm:p-4 flex flex-col flex-grow">
                    ...
                    <div className="flex items-center gap-1 mb-3">
                      ...
                      <span className="text-[11px] font-medium text-muted-foreground">
                        {(
                          prod as Product & { stats?: { averageRating?: number } }
